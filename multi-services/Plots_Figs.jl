...
function plot_convergence(Obj1, Obj2, rs_Obj, r1, r2, eta1, eta2, rs_eta, w1, w2, ws2, rs_w, f1, f2, rs_f, stop1, stop2)
    ...
    ax = subplot(1,1,1)
    ax.tick_params("both",labelsize =legend_fontsize-1)
    plot(r1[1,1,1:stop2[1]],label="UE1:UE50")
    for n=2:NumbDevs
    ...
    ax.tick_params("both",labelsize =legend_fontsize-1)
    for s=1:Numb_Services
        plot(r2[1,s,1,1:stop2[1]],color=colors1[s],label=string("Service ",s)*", UE1:UE5")
        for n=2:5
            plot(r2[1,s,n,1:stop2[1]],color=colors1[s])
        end
    end
    ...
    ax = subplot(1,1,1)
    ax.tick_params("both",labelsize =legend_fontsize-1)
    w_sorted = partialsortperm(rs_w,1:5,rev=true)
    println(w_sorted)
    # for n=40:NumbDevs
    for n in w_sorted
        if(n==48)
            plot(rs_w[n]*ones(t),label="Solver",linestyle=":", marker=markers[1], color="k",markevery=5)
            # plot(w1[1,n,1:3], color="r",label="Centralized",linestyle="-", marker=markers[3], markersize=marker_size,markevery=Stride1)
            plot(w2[1,n,1:stop2[1]],label=string("Decentralized:UE",n),linestyle="-", color="r")
        else
            plot(rs_w[n]*ones(t),linestyle=":", marker=markers[1], color="k",markevery=5)
            # plot(w1[1,n,1:3],color="r",linestyle="-", marker=markers[3], markersize=marker_size,markevery=Stride1)
            # if(n==41)
            plot(w2[1,n,1:stop2[1]], linestyle="-", label=string("Decentralized:UE",n))
            # else
            #     plot(w2[1,n,1:stop2[1]], linestyle="-", color=colors1[1])
            # for s=1:Numb_Services
...
function plot_comparison(rs_Obj, Obj_E, Obj_T, Heuristic_Obj,Heuristic_Obj_E, Heuristic_Obj_T, Heuristic_Obj1,Heuristic_Obj_E1, Heuristic_Obj_T1,
    ...
    width = 0.25
    barlist1 = bar(idx.-width, K_g.*Heuristic_Obj_E, width, color=colors1[1], alpha=.8, hatch=patterns[1], label=alg_labels[1])
    barlist2 = bar(idx, K_g.*Heuristic_Obj_E1, width, color=colors1[2], alpha=.8, hatch=patterns[2], label=alg_labels[2])
    barlist3 = bar(idx.+width, K_g.*Obj_E, width, color=colors1[3], alpha=.8, hatch=patterns[3], label=alg_labels[3])
    ...
    # idx = [1:1:Numb_Services;]
    # width = 0.25
    barlist1 = bar(idx.-width, K_g.*Heuristic_Obj_T, width, color=colors1[1], alpha=.8, hatch=patterns[1], label=alg_labels[1])
    barlist2 = bar(idx, K_g.*Heuristic_Obj_T1, width, color=colors1[2], hatch=patterns[2], alpha=.8, label=alg_labels[2])
    barlist3 = bar(idx.+width, K_g.*Obj_T, width, color=colors1[3], alpha=.8, hatch=patterns[3], label=alg_labels[3])
    ...
    barlist1 = bar(idx.-width, K_g.*(Heuristic_Obj_E + kaps[1]*Heuristic_Obj_T), width, color=colors1[1], alpha=.8, hatch=patterns[1], label=alg_labels[1])
    barlist2 = bar(idx, K_g.*(Heuristic_Obj_E1 + kaps[1]*Heuristic_Obj_T1), width, color=colors1[2], hatch=patterns[2], alpha=.8, label=alg_labels[2])
    barlist3 = bar(idx.+width, K_g.*(Obj_E + kaps[1]*Obj_T), width, color=colors1[3], alpha=.8, hatch=patterns[3], label=alg_labels[3])
...
function plot_pareto(rs_T_cmp1, rs_E_cmp1, rs_T_com1, rs_E_com1, rs_eta1)
    ...
    scatter(Obj_E, Obj_T)
    for (k, v) in enumerate(kaps_pareto)
        # ax.text(Obj_E[k], Obj_T[k]+25, "%d" %v, ha="center")
        ax.annotate(string("\$\\kappa_s\$=",v), xy=(Obj_E[k],Obj_T[k]), xytext=(3,1.5), textcoords="offset points",fontsize=11)
    end
