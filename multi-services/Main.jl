...
function BCD(dist_list, gain_list, capacity, D_n)
    ...
    for k=1:Numb_kaps
        ...
        println("* mean BCD:",mean(times))
        println("* median BCD:",median(times))
   end
...
function ADMM(dist_list, gain_list, capacity, D_n, jpadmm=false, early_stop=false)
    ...
    for k=1:Numb_kaps
        ...
        println("* mean ADMM:",mean(times))
        println("* median ADMM:",median(times))
   end
...
function main()
    ...
    # t_record = @timed  ADMM(dist_list, gain_list, capacity, D_n,jpadmm,false)
    # println("Running time of Decentralized ADMM:", t_record[2])
    # t_record = @timed  ADMM(dist_list, gain_list, capacity, D_n,jpadmm,true)  #early_stop
    # println("Running time of Decentralized ES:", t_record[2])
